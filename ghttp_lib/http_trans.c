...
http_trans_conn *http_trans_conn_new(void)
{
	...
	/* default to CONN_PORT_DEFAULT */
	l_return->port = CONN_PORT_DEFAULT;
	/* default to IO_BUF_CHUNKSIZE_DEFAULT bytes at a time */
	l_return->io_buf_chunksize = CONN_IO_BUF_CHUNKSIZE_DEFAULT;
...
int http_trans_write_buf(http_trans_conn *a_conn)
{
	...
	/* write out some data */
	if ((a_conn->last_read = l_written = write (a_conn->sock, &a_conn->io_buf[a_conn->io_buf_io_done],
