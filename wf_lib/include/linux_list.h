...
#define SLIST_HEAD(name) struct hlist_head name = { .head = NULL, .tail = NULL }

#define INIT_SLIST_HEAD(list) do { \
...
static inline int slist_empty_careful(const struct slist_head *list)
{
	return (list->head == NULL) || (list->tail == NULL);
}

static inline void slist_add(struct slist_head *list, struct slist_node *newNode)
{
	newNode->next = NULL;
...
static inline void slist_add_head(struct slist_head *list, struct slist_node *newNode)
{
...
#define slist_while_get_head(pos, list)	\
	for (pos = slist_del_get_head(list); pos != NULL; slist_del_get_head(list))

#define slist_del_get_head_entry(list, type, member)	\
	(((list)->head) ? slist_entry(slist_del_get_head(list), type, member) : NULL)
...
static inline void slist_splice(struct slist_head *newList, struct slist_head *list)
{
	if (!slist_empty_careful(newList))
		__slist_splice(newList, list);
}

static inline void slist_splice_init(struct slist_head *newList, struct slist_head *list)
{
	if (!slist_empty_careful(newList))
	{
		__slist_splice(newList, list);
		INIT_SLIST_HEAD(newList);
	}
}

/*
#define slist_for_each(pos, list) \
	for (pos = (list)->head; prefetch(pos->next), pos != NULL; pos = pos->next)

#define __slist_for_each(pos, list)	for (pos = (list)->head; pos != NULL; pos = pos->next)
*/
#define slist_for_each(pos, list) \
...
#define slist_for_each_entry(pos, list, member)				\
	for (pos = slist_entry((list)->head, typeof(*pos), member);	\
	     prefetch(pos->member.next), &pos->member != NULL; 	\
