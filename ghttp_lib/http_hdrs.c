...
int http_hdr_set_value_no_nts(http_hdr_list *a_list, const char *a_name_start, int a_name_len,
	const char *a_val_start, int a_val_len)
{
	int l_return = 0;
	...
	if ((a_list == NULL) || (a_name_start == NULL) || (a_val_start == NULL) || (a_name_len == 0))
		goto ec;
...
int http_hdr_set_value(http_hdr_list *a_list, const char *a_name, const char *a_val)
{
	int i = 0;
	char *l_temp_value = NULL;
	int l_return = 0, index;
  
	if ((a_list == NULL) || (a_name == NULL) || (a_val == NULL))
		goto ec;
	l_temp_value = http_hdr_get_value_index(a_list, a_name, &index);
	...
	{
		free(a_list->value[index]);
		a_list->value[index] = strdup(a_val);
		l_return = 1;
	}
ec:
	return l_return;
...
char *http_hdr_get_value_index(http_hdr_list *a_list, const char *a_name, int *index)
{
	int i = 0;
	char *l_return = NULL;
...
char *http_hdr_get_value(http_hdr_list *a_list, const char *a_name)
{
	return http_hdr_get_value_index(a_list, a_name, NULL);
}
