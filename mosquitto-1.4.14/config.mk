...
LIB_CFLAGS:=${CFLAGS} ${CPPFLAGS} -I. -I.. -I../lib -I${OPENSSL_INSTALL}/include
LIB_CXXFLAGS:=$(LIB_CFLAGS) ${CPPFLAGS}
LIB_LDFLAGS:=${LDFLAGS} -L${OPENSSL_INSTALL}/lib 

BROKER_CFLAGS:=${LIB_CFLAGS} ${CPPFLAGS} -DVERSION="\"${VERSION}\"" -DTIMESTAMP="\"${TIMESTAMP}\"" -DWITH_BROKER -I${OPENSSL_INSTALL}/include
CLIENT_CFLAGS:=${CFLAGS} ${CPPFLAGS} -I../lib -DVERSION="\"${VERSION}\"" -I${OPENSSL_INSTALL}/include
...
CLIENT_LDFLAGS:=$(LDFLAGS) -L../lib ../lib/libmosquitto.so.${SOVERSION} -L${OPENSSL_INSTALL}/lib -lcrypto -lssl
...
ifeq ($(WITH_TLS),yes)
	BROKER_LIBS:=$(BROKER_LIBS) -L${OPENSSL_INSTALL}/lib -lssl -lcrypto
	LIB_LIBS:=$(LIB_LIBS) -L${OPENSSL_INSTALL}/lib -lssl -lcrypto
	BROKER_CFLAGS:=$(BROKER_CFLAGS) -DWITH_TLS 
	LIB_CFLAGS:=$(LIB_CFLAGS) -DWITH_TLS 
	PASSWD_LIBS:=-L${OPENSSL_INSTALL}/lib -lcrypto
