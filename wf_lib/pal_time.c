...
#define WRAPAROUND_VALUE (0xffffffffUL / HZ + 1) /* HZ = frequency of ticks per second. */

/* Static function to get current sec and usec.  */
static int system_uptime (struct pal_timeval *tv, struct pal_tzval *tz)
{
	struct sysinfo info;
...
void pal_time_tzcurrent (struct pal_timeval *tv, struct pal_tzval *tz)
{
	system_uptime (tv, tz);
	return;
}
...
*/
int pal_time_gmt (pal_time_t * tp, struct pal_tm *gmt)
{
	struct pal_tm *tmp;
...
**   RESULT_OK for success, else the error which occurred
*/
int pal_time_loc (pal_time_t * tp, struct pal_tm *loc)
{
	struct pal_tm *tmp;
	/*
...
*/
int pal_time_calendar (const pal_time_t *tp, char *buf) 
{
	strcpy (buf, ctime (tp));
	return RESULT_OK;
}
...
**   RESULT_OK for success, else the error which occurred
*/
int pal_delay (pal_time_t t_usec)
{
	int ret;
